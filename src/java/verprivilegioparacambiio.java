 ...
 * @author kazki
 */
@WebServlet(urlPatterns = {"/verprivilegioparacambiio"})
public class verprivilegioparacambiio extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet verprivilegioparacambiio</title>"); 
         ...
         String  nombre= request.getParameter("nombre");
            ...
            out.println("<body>");
             try
             {
            String logear2 ="select Privilegio from usuarios where usuario='"+nombre1+"'";
            Statement a=conexion.createStatement();
             ResultSet fila=a.executeQuery(logear2);
             String privilegio1 = String.valueOf(privilegio);
             int privi = Integer.parseInt(privilegio1);
      if( privilegio.equals("1")){
    	request.getRequestDispatcher("efectuarcambiodatos").forward(request, response);
        String nombre2 = nombre;
      }
      else if(fila.getInt("privilegio") < privi){
    request.getRequestDispatcher("efectuarcambiodatos").forward(request, response);
        String nombre2 = nombre;
      }
                   }
